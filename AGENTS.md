...
**Aruba ClearPass Policy Manager (NAC) module.**

- **Repo:** `github.com/lbockenstedt/cppm`
- **Module type:** `module_type = "nac"`
- **Canonical docs:** [`lm/docs/cppm.md`](../lm/docs/cppm.md) *(in the `lm` repo — the master registry)*
...
`src/spoke.py` (spoke), `src/control_plane.py` (hub link), `src/client.py` (ClearPass API
client), `src/queries.py` (the query set). `API_SPEC.md` documents the surface.

## cppm-specific gotchas

- Hub-side context lives in `lm/clearpass/` — including `netbox-tenant-context-server-action.json`, which wires ClearPass enforcement to NetBox tenancy. Changes to tenant handling usually touch both.
